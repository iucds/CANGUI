...
//	  }
//		while (head != tail){
//			usebuffer = circular_buffer[tail];
//			serial_to_can(usebuffer,huart1,txHeader,canMailbox,&hcan);
//			HAL_UART_Transmit(&huart1,usebuffer,strlen(usebuffer),buffer_size);
//			memset(circular_buffer[tail],0,sizeof(circular_buffer[tail]));
//			tail ++;
//			if (tail == buffers) tail = 0;
		//		HAL_Delay(1000);
//		  }
    /* USER CODE BEGIN 3 */
  }
...
{
	UART_HandleTypeDef huart[] = {huart1,huart2,huart3};
